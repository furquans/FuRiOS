...
put_no_oob() 
{
	...
	image_pages=`echo "$image_len/2048" | bc`

	if [ 0 -ne `echo "$image_len%$flash_page_size" | bc` ]; then
		image_pages=`echo "$image_pages+1" | bc`
	fi
	...
	do
  	#echo $i
  	    out_offset=`echo "$image_page_offset+$i" | bc`
  	    in_offset=$i
  	#echo "out_offset:"$out_offset
  	#echo "in_offset:"$in_offset
  	dd if=$image_name of=$flash_image_name conv=notrunc count=1 obs=`echo $flash_page_size + $flash_oob_size | bc` ibs=$flash_page_size  seek=$out_offset skip=$in_offset  
  	i=`echo "$i + 1" | bc`
	done	
}
